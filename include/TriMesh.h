...
public:
    TriMesh();
	TriMesh(Model* model) { this->model = model; }
	~TriMesh();
    static TriMesh* generateTriMesh_by_aiMesh(aiMesh* mesh,const aiScene* scene,Model* model,bool isbound,
    ...
    std::vector<glm::ivec4> getVertexBoneID();
    std::vector<glm::vec4> getVertexWeight();
    ...
    std::vector<glm::ivec4> getBoneID();
    std::vector<glm::vec4> getWeight();
    std::vector<Texture> getTexturesMessage();
...
protected:
    ...
    std::vector<Texture> textures_message; //纹理信息
    ...
    std::vector<glm::ivec4> vertex_m_BoneID;//顶点对应的骨骼ID
    std::vector<glm::vec4> vertex_m_Weight;//顶点对应的骨骼权重
    ...
    std::vector<vec3i> boneID_index;    //每个三角面片的顶点对应的骨骼ID的下标
    std::vector<vec3i> weight_index;    //每个三角面片的顶点对应的骨骼权重的下标
    ...
    std::vector<glm::ivec4> m_BoneID;//传入着色器的骨骼ID
    std::vector<glm::vec4> m_Weight;//传入着色器的骨骼权重
